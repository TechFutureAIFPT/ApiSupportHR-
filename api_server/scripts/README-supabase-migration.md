# SupportHR Firebase to Supabase migration
...
```powershell
python scripts/firebase_supabase_migration.py export --output D:\SupportHR-Migration-Backups\supporthr-final.enc
```

After Auth import and SQL migration:

```powershell
python scripts/firebase_supabase_migration.py import --archive D:\SupportHR-Migration-Backups\supporthr-final.enc
python scripts/firebase_supabase_migration.py reembed
python scripts/firebase_supabase_migration.py reconcile --archive D:\SupportHR-Migration-Backups\supporthr-final.enc
```
...
The final maintenance-window run repeats export/import/reconcile after writes and workers are stopped. Do not set `AUTH_PROVIDER=supabase` or `DATA_PROVIDER=supabase` until reconciliation succeeds.
