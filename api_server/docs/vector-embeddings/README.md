...
The production backend uses Firestore native vector search for approved RAG exemplars. The active contract is:
...
- a Firestore `Vector` in `embedding`.
...
rubric, and vector-version prefilters and calls Firestore `find_nearest` with cosine distance.
...
`VECTOR_STORE_PROVIDER=firestore`. JSON vectors are only accepted for local/test use when they explicitly
match the active contract; legacy JSON files without version metadata are ignored.
