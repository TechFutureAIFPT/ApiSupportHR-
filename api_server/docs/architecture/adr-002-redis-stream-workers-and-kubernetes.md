...
The first async CV endpoint used `asyncio.create_task` inside the API process. Firestore snapshots made job
...
- Store short-lived internal payload/state in Redis and authenticated audit snapshots in Firestore.
...
- Firestore remains the record system; this change does not introduce a relational transaction boundary.
