...
def value(row: dict[str, Any], *aliases: str) -> str:
    normalized = {
        re.sub(r"[^a-z0-9]+", "_", str(key).lower()).strip("_"): item
...
def records(csv_path: Path, *, status: str, limit: int | None) -> Iterable[dict[str, Any]]:
    with csv_path.open("r", encoding="utf-8", errors="ignore", newline="") as handle:
        ...
        for row_number, row in enumerate(csv.DictReader(handle), start=2):
            ...
            source = f"{csv_path.name}:{row_number}:{redacted[:500]}:{jd[:500]}"
            doc_id = hashlib.sha256(source.encode()).hexdigest()[:32]
            yield {
                "id": doc_id,
...
def main() -> int:
    ...
    parser.add_argument("--project", default=os.getenv("GOOGLE_CLOUD_PROJECT") or os.getenv("FIREBASE_PROJECT_ID"))
    ...
    if pending:
        batch.commit()
    print(f"Seeded {total} {args.status} records into Firestore collection {args.collection}.")
    return 0
